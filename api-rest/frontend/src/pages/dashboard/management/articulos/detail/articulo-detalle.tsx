import { Alert, Button, Card, Group, Loader, Stack, Text, Image } from '@mantine/core';
...
export default function ArticuloDetalle({ articuloId }: { articuloId: string }) {
  const { data: articulo, isLoading, error } = useGetArticulo({ route: { id: articuloId } });

  if (isLoading) return <Loader />;
  if (error) return <Alert color="red" maw={400}>Error al cargar articulo</Alert>;
