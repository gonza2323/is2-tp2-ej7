...
import { AutorSummaryDto } from '@/api/dtos';
...
type SortableFields = AutorSummaryDto;
...
export function AutoresTable() {
  ...
  const columns: DataTableColumn<AutorSummaryDto>[] = useMemo(
    () => [
      {
        accessor: 'apellido',
        title: 'Apellido',
        sortable: true,
        render: (autor) => (
          <Text truncate="end">{autor.apellido}</Text>
        ),
      },
      {
        accessor: 'nombre',
        ...
        sortable: true,
        render: (autor) => (
          <Text truncate="end">{autor.nombre}</Text>
        ),
      },
      {
        accessor: 'biografia',
        ...
        sortable: false,
        render: (autor) => (
          <Text truncate="end">{autor.biografia}</Text>
        ),
      },
      {
        accessor: 'actions',
        ...
        render: (autor) => (
          <DataTable.Actions
            onView={() => { navigate(paths.dashboard.management.autores.view(autor.id)) }}
            onEdit={() => { navigate(paths.dashboard.management.autores.edit(autor.id)) }}
  ...
  const handleDelete = (autor: AutorSummaryDto) => {
  ...
  return (
    <DataTable.Container>
      <DataTable.Title
        ...
        actions={
          <AddButton variant="default" size="xs" component={NavLink} to={paths.dashboard.management.autores.add}>
            Agregar autor
          </AddButton>
        }
      />
