...
import { LibroSummaryDto } from '@/api/dtos';
...
type SortableFields = LibroSummaryDto;
...
export function LibrosTable() {
  ...
  const columns: DataTableColumn<LibroSummaryDto>[] = useMemo(
    () => [
      {
        accessor: 'titulo',
        title: 'Título',
        sortable: true,
        render: (autor) => (
          <Text truncate="end">{autor.titulo}</Text>
        ),
      },
      {
        accessor: 'fecha',
        ...
        sortable: true,
        render: (autor) => (
          <Text truncate="end">{autor.fecha}</Text>
        ),
      },
      {
        accessor: 'genero',
        ...
        sortable: true,
        render: (autor) => (
          <Text truncate="end">{autor.genero}</Text>
        ),
      },
      {
        accessor: 'paginas',
        ...
        sortable: true,
        render: (autor) => (
          <Text truncate="end">{autor.paginas}</Text>
        ),
      },
      {
        accessor: 'actions',
        ...
        render: (autor) => (
          <DataTable.Actions
            onView={() => { navigate(paths.dashboard.management.libros.view(autor.id)) }}
            onEdit={() => { navigate(paths.dashboard.management.libros.edit(autor.id)) }}
  ...
  const handleDelete = (autor: LibroSummaryDto) => {
  ...
  return (
    <DataTable.Container>
      <DataTable.Title
        ...
        actions={
          <AddButton variant="default" size="xs" component={NavLink} to={paths.dashboard.management.libros.add}>
            Agregar autor
          </AddButton>
        }
      />
