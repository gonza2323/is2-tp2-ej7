import { Box, Button, Grid, Group, MultiSelect, NumberInput, Select, TextInput } from '@mantine/core';
...
export default function LibroCreateForm() {
  ...
  const autoresData = autores?.map(a => ({ value: String(a.id), label: a.nombre })) || [];
  const personasData = personas?.map(p => ({ value: String(p.id), label: `${p.nombre} ${p.apellido}` })) || [];
  ...
  const handleSubmit = form.onSubmit((values) => {
    createLibro.mutate(
      { variables: values }, // values are already transformed
      {
        onSuccess: () => {
          notifications.show({
            title: "Éxito",
