...
import { ArticuloSummaryDto } from '@/api/dtos';
...
type SortableFields = Pick<ArticuloSummaryDto, 'nombre'>;
...
export function ArticulosTable() {
  ...
  const columns: DataTableColumn<ArticuloSummaryDto>[] = useMemo(
    () => [
      {
        accessor: 'nombre',
        title: 'Nombre',
        sortable: true,
        render: (articulo) => (
          <Text truncate="end">{articulo.nombre}</Text>
        ),
      },
      {
      ...
      },
      {
        ...
        render: (articulo) => (
          <DataTable.Actions
            onView={() => { navigate(paths.dashboard.management.articulos.view(articulo.id)) }}
            onEdit={() => { navigate(paths.dashboard.management.articulos.edit(articulo.id)) }}
  ...
  const handleDelete = (articulo: ArticuloSummaryDto) => {
  ...
  return (
    <DataTable.Container>
      <DataTable.Title
        ...
        actions={
          <AddButton variant="default" size="xs" component={NavLink} to={paths.dashboard.management.articulos.add}>
            Agregar articulo
          </AddButton>
        }
      />
